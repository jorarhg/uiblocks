...
exports.HybridFilterAnalyzer = void 0;
class HybridFilterAnalyzer {
    constructor(config) {
        this.globalConfig = {
            facetedThreshold: 20,
            searchMinLength: 2,
     ...
     */
    applyStrategy(strategy, column, reasoning) {
        switch (strategy) {
            case 'column-meta':
     ...
     */
    analyzeColumnMetadata(column, reasoning) {
     ...
     */
    analyzeConfigOverride(column, reasoning) {
     ...
     */
    analyzeTypeInference(column, reasoning) {
        const columnDef = column.columnDef;
        ...
        const typeHints = this.inferTypeFromAccessorKey(accessorKey);
     ...
     */
    analyzeDataPattern(column, reasoning) {
     ...
     */
    performDataAnalysis(values, reasoning) {
     ...
     */
    analyzeBooleanData(values, reasoning) {
     ...
     */
    analyzeNumericData(values, reasoning) {
        ...
        if (numericRatio >= 0.8) {
            ...
            const isSequential = sortedNumbers.every((num, idx) => idx === 0 || num === sortedNumbers[idx - 1] + 1);
     ...
     */
    analyzeDateData(values, reasoning) {
        let dateCount = 0;
     ...
     */
    analyzePatterns(values, reasoning) {
        const patterns = this.globalConfig.patterns;
        if (!patterns)
            return null;
        for (const [patternName, pattern] of Object.entries(patterns)) {
            if (patternName === 'date')
                continue; // Ya manejado en analyzeDateData
            let matchCount = 0;
            for (const value of values) {
                const strValue = String(value);
     ...
     */
    findPatternInName(columnName) {
        const lowerName = columnName.toLowerCase();
        // Patrones comunes en nombres de columnas
        const namePatterns = [
            { pattern: /id$/i, config: { type: 'search', priority: 'medium' } },
            { pattern: /email/i, config: { type: 'search', priority: 'medium' } },
            { pattern: /phone/i, config: { type: 'search', priority: 'medium' } },
            { pattern: /url/i, config: { type: 'search', priority: 'medium' } },
            { pattern: /date|time/i, config: { type: 'range', priority: 'high' } },
            { pattern: /price|cost|amount/i, config: { type: 'range', priority: 'high' } },
            { pattern: /count|quantity|number/i, config: { type: 'range', priority: 'medium' } },
            { pattern: /status|state|type|category/i, config: { type: 'faceted', priority: 'high' } },
            { pattern: /active|enabled|visible|public/i, config: { type: 'boolean', priority: 'high' } }
        ];
     ...
     */
    inferTypeFromAccessorKey(accessorKey) {
        const hints = [];
        const key = accessorKey.toLowerCase();
        // Mapeo de sufijos/prefijos a tipos
        const typeMap = [
            { pattern: /id$/i, type: 'search', confidence: 0.7 },
            { pattern: /name|title|description/i, type: 'search', confidence: 0.8 },
            { pattern: /date|time|created|updated/i, type: 'range', confidence: 0.85 },
            { pattern: /price|cost|amount|total|count/i, type: 'range', confidence: 0.9 },
            { pattern: /status|state|type|category|priority/i, type: 'faceted', confidence: 0.85 },
            { pattern: /active|enabled|visible|public|private/i, type: 'boolean', confidence: 0.9 }
        ];
     ...
     */
    createResultFromConfig(column, config, strategy, confidence) {
     ...
     */
    createMetadata(column, strategy, confidence) {
     ...
     */
    inferDataTypeFromFilterType(filterType) {
        switch (filterType) {
            case 'range':
...
exports.HybridFilterAnalyzer = HybridFilterAnalyzer;
